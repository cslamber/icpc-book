...
using T=modnum;using u64=uint64_t;using u32=uint32_t;
u32 v;
static u32 minv(u32 a,u32 m){a%=m;assert(a);return a==1?1:u32(m-u64(minv(m,a))*u64(m)/a);}
modnum():v(0){}
modnum(int _v){_v%=MOD;v=_v<0?_v+MOD:_v;}
explicit operator u32()const{return v;}
friend ostream&operator<<(ostream&out,const T&n){return out<<u32(n);}
friend istream&operator>>(istream&in,T&n){int v_;in>>v_;n=modnum(v_);return in;}
...
T&operator*=(const T&o){v=u32(u64(v)*u64(o.v)%MOD);return*this;}
